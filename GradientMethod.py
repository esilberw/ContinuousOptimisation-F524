...
class GradientMethod:
    ...
    def csv_to_numpy_array_dataset(self):
        ...
        self.dataset = df.astype(np.float64).to_numpy()
    ...
    def init_regression(self):
        ...
        self.X = self.dataset[:, :-1]  # Toutes les colonnes sauf la dernière
        self.y = self.dataset[:, -1]  # Dernière colonne

        # Add a col of "1" elements x_0j correspind to Betha_0 (interception)
        X_b = np.hstack((np.ones((self.X.shape[0], 1)), self.X))

        self.beta_hat = np.linalg.pinv(X_b) @ self.y

    def gradient_descent(self, t=0.01, n_iterations=1000, tolerance=1e-6):
        """
...
dataset_file = "./Datasets/wine+quality/winequality-red.csv"
